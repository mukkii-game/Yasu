...
9. The kana grid offers 「ボ」 where 「ホ」 used to be, so 「ボス」 can be entered, and the three letters either ending needs — 「ヤ」「ス」「ボ」 — are drawn one pixel thicker than the rest as a nudge. Naming the boss opens Yasu's own ending instead of the arrest, over six pages. Each page states its own motion rather than inheriting the room's tremble, so the script alone decides when he moves. `ヤス「ボスがはんにん？` / `ごじょうだんを　はっはっは」` opens on the supplied 間抜け7 effect with him already laughing: only the torso bobs, exactly as on the reward line, and nothing trembles underneath it. It carries the route's one comeback, 「ジョークとして」 / 「ヤスッ！」, on the ordinary two-beat rhythm; the pages after it play straight. `ヤス「でも　じはくとして` / `ろくおんさせてもらいました」` sounds the unease cue Yasu first walked in on from its very first character rather than once the line has landed, and he holds completely still under it — no laugh, no tremble — while the room stays steady too. `ヤス「これ　そのままじじつに` / `させてもらいますわ」` opens on the supplied dissonant chord, and the faint tremble returns for it and for the rest of the route. `ヤス「はんにん　ていこうのため` / `やむなくせいあつ　ってね」` opens on the supplied machine-gun magazine release with the unease cue under it, as a plain round muzzle appears beside Yasu, half clear of his jacket, and it stays up for the rest of the route. He gives a single nod once `ヤス「ボス　いままでありがとう」` has landed, then signs off with `ヤス「そしてさようなら」`. Advancing past that runs the rest unattended, with all input ignored: the supplied `ドーン.mp3` fires 700 ms later as the muzzle flares white and kicks back two pixels before settling, and the room floods red over 1.6 seconds. `THE END` appears 800 ms after the flood completes, so the red is left to stand on its own for a beat first, and 「ボスのいのち」 / 「ヤスッ！」 then lands on the ordinary two-beat rhythm, at the same 60-pixel size and the same height on screen as the arrest ending's payoff and on that ending's `ドーン.mp3` rather than the room's ordinary comeback effect, so both routes close on the same sound. The shot earlier in this sequence uses it too, four and a half seconds before. The screen fades and returns to the title on its own. Any other two characters are still refused as before.
...
- There is no sound control. The supplied `不安（ピアノ演奏）.mp3` starts once at the beginning of the opening Yasu dialogue. Character-by-character dialogue has a square-wave blip at 0.026 gain, a step above the 0.018 first punchline cue, because every supplied effect peaks 14 to 29 times higher and the typing cue was disappearing under them. Kana choices make a short click and 「けってい」 makes a two-note confirmation. Ordinary dialogue navigation remains silent; the reveal, both halves of each punchline, and the finale have their own effects. The second punchline cue remains intentionally much louder.
