...
7. `ヤス「まあ しょはんだし すぐでてこれますよねｗ」` carries no comeback of its own. It is answered instead by the last ending page, the player's correction `あなた「ごうとうさつじんは` / `しけいか　むきちょうえきだぞ？」`, with a hard break before 「しけいか」. The moment that line finishes typing, the supplied `ショック1.mp3` effect plays and Yasu shakes by four logical pixels for exactly one second, identical to the reveal; the line itself stays on screen throughout and input is ignored. Its comeback then follows on the ordinary rhythm and with the ordinary cues, since the gag only reads as a repeat when it sounds like one.
...
9. Taps, clicks, and unmodified keys are ignored throughout `THE END`, preventing the player from skipping the final joke. Input becomes active again only after the automatic return to a clean title state; no retry button is shown.
...
- While the completed 「まあ しょはんだし」 page is visible, only Yasu's head nods down and up twice; the body remains still.
