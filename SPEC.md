...
- The story screen uses a broad 236×150 framed room scene above an 80-pixel-tall dialogue area. There is no unused command list. The room uses flat color blocks, a barred window, and no desk. It contains only one centered, front-facing Yasu built in the supplied FC portrait's layered style: a common black silhouette outlines both the softly rounded angular face and the shoulder-shaped torso; muted yellow skin and a continuous black center-part with an octagonal cap fill the head; separate bright-blue jacket panels, navy lapels, a full-height white shirt, black-edged white collars, and a black-edged 26-pixel red tie fill the torso. Smaller white 「ヤ」「ス」 lettering stays between hair and chin. The title uses the same construction and colors at approximately half scale, omitting only the face letters. The face has no eyes or mouth. There is no boss or foreground player. Yasu stops trembling once the first punchline appears. While 「はっはっは」 is visible, only his torso bobs vertically by two pixels.
