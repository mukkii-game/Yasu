...
- The story screen uses a broad 236×150 framed room scene above an 80-pixel-tall dialogue area. There is no unused command list. The room uses flat color blocks, a barred window, and no desk. It contains only one centered, front-facing Yasu: a blocky 7:3 side part whose left fringe descends in steps, a 50-pixel round blank head with smaller white 「ヤ」「ス」 lettering confined below the hair and above the chin, a black square body reaching the picture bottom, and a short white tie. The lettering has no outline, and the face has no eyes or mouth. There is no boss or foreground player. Yasu stops trembling once the first punchline appears.
