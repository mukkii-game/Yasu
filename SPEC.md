...
- The story screen uses a broad 236×150 framed room scene above an 80-pixel-tall dialogue area. There is no unused command list. The room uses flat color blocks, a barred window, and no desk. It contains only one centered, front-facing Yasu built from deliberately coarse FC-scale shapes: a 50-pixel round muted-yellow face with a two-pixel black outline, black center-part hair made from a few rectangular steps, a plain rectangular navy torso, and one short white tie. There are no jacket outlines, lapels, collars, shirt panel, or red tie. Smaller white 「ヤ」「ス」 lettering stays between hair and chin. The title uses the same simple construction at approximately half scale, omitting only the face letters. The face has no eyes or mouth. There is no boss or foreground player. Yasu stops trembling once the first punchline appears. While 「はっはっは」 is visible, only his torso bobs vertically by two pixels.
