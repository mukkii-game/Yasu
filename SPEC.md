...
9. The kana grid offers 「ボ」 where 「ホ」 used to be, so 「ボス」 can be entered, and the three letters either ending needs — 「ヤ」「ス」「ボ」 — are drawn one pixel thicker than the rest as a nudge. Naming the boss opens Yasu's own ending instead of the arrest, over six pages, with Yasu trembling faintly throughout. `ヤス「ボスがはんにん？` / `ごじょうだんを　はっはっは」` opens with him already laughing, the same torso bob the reward line uses, which stops on the next page. `ヤス「でも　じはくとして` / `ろくおんさせてもらいました」` finishes typing and the unease cue Yasu first walked in on returns. `ヤス「それ　そのままじじつに` / `させてもらいますわ」` gets no movement beyond the tremble. `ヤス「はんにん　ていこうのため` / `やむなくせいあつ　ってね」` opens on a synthesized clack as a plain round muzzle appears beside Yasu, half clear of his jacket, and it stays up for the rest of the route. He gives a single nod once `ヤス「ボス　ありがとう」` has landed, then signs off with `ヤス「そしてさようなら」`. Advancing past that runs the rest unattended, with all input ignored: the supplied `ドーン.mp3` fires 700 ms later as the muzzle flares white and kicks back two pixels before settling, and the room floods red over 1.6 seconds. `THE END` appears on the red once the flood completes, and 「ボスのいのち」 / 「ヤスッ！」 then lands on the ordinary two-beat rhythm with the ordinary comeback effect, at the same 60-pixel size and the same height on screen as the arrest ending's payoff. The screen fades and returns to the title on its own. Any other two characters are still refused as before.
